...
export default function Sidebar() {
  ...
  return (
    <aside
      className={clsx(
        "fixed flex flex-col top-0 left-0 h-full w-64 p-4 bg-surface-elevated gap-10 transform transition-transform duration-300",
        isOpen ? "translate-x-0 shadow-2xl" : "-translate-x-full"
      )}
    >
      <div className="flex justify-between items-center">
        <Hexagon onClick={close} className="w-6 h-6 text-primary" />
        <X onClick={close} className="w-5 h-5 text-primary" />
      </div>
      ...
      <div>
        <p className="text-secondary font-light">Your chats</p>
        <div>
          {chats.map((chat) => (
            <div
              key={chat.id}
              ...
              className={clsx(
                "relative group flex items-center justify-between px-3 py-2 rounded-lg hover:bg-surface-accent font-normal cursor-pointer",
                chat.id === activeChatId ? "bg-surface-accent" : "bg-transparent"
              )}
            >
              {chat.title}
              <MoreHorizontal
              ...
              {openMenuId === chat.id && (
                <div
                  ...
                  className="absolute flex flex-col gap-1 left-8 top-10 p-1 w-auto bg-surface border border-strong rounded-2xl shadow-lg z-50"
                >
      ...
      <div className="fixed flex items-center justify-between bottom-0 left-0 right-0 p-4 border-t border-subtle bg-surface-elevated">
